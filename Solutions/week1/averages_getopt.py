...
def main(*arguments):
    ...
    flags, positional_arguments = handle_options_w_getopt(arguments)
...
# Docs: https://docs.python.org/3/library/getopt.html#module-getopt
def handle_options_w_getopt(arguments):
    #
    try:
        opts, args = getopt.getopt(arguments, "", ["help",
                                                   "mean",
                                                   "median",
                                                   "mode",
                                                   "file="])
    ...
    for opt, arg in opts:
        if opt == '--help':
            print_usage(1)
        elif opt in ("-m", "--mean", "--median", "--mode"):
            flags.append(opt)
        elif opt in ("-f", "--file"):
            flags.append(opt)
            positional_arguments.append(arg)
    ...
    if "--file" not in flags:
        ...
        #
        if len(flags) == 0:
            flags.append(["--mean", "--median", "--mode"])
    return flags, positional_arguments


def print_usage(exit_code):
    print(f'usage: {sys.argv[0]} '
          f'[[--mean --media --mode] num1 num2 ...] [--file filename]')
    sys.exit(exit_code)
